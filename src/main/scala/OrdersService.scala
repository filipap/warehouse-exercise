...
class OrdersService {
	...
	var dao : OrdersGenerator = new OrdersGenerator()
	var allOrders : List[Order] = dao.loadData()

	def filterOrdersBySpecificInterval(monthInit: Integer, monthEnd: Integer) : scala.collection.mutable.HashMap[String, Int] = {
	...
	def filterAllOrders (dateInit: LocalDateTime, dateEnd: LocalDateTime): HashMap[String,Int] = {
        var result: HashMap[String,Int] = HashMap(_1_3_MONTHS -> 0, _4_6_MONTHS -> 0, _7_12_MONTHS -> 0, _12_MONTHS -> 0)
